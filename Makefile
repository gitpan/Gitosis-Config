# This Makefile is for the Gitosis::Config extension to perl.
#
...
#     ABSTRACT => q[Parse and Write gitosis config files]
#     AUTHOR => q[Chris Prather  C<< <perigrin@cpan.org> >>]
#     DIR => []
#     DISTNAME => q[Gitosis-Config]
#     NAME => q[Gitosis::Config]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0.42], MooseX::AttributeHelpers=>q[0.13], ExtUtils::MakeMaker=>q[6.42], Config::INI=>q[0.013], Moose=>q[0.56] }
#     VERSION => q[0.0.2]
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     test => { TESTS=>q[t/00.load.t t/01.parse.t t/02.output.t] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Gitosis/Config.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Gitosis
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Gitosis
...
TO_INST_PM = lib/Gitosis/Config.pm \
	lib/Gitosis/Config/Group.pm \
	lib/Gitosis/Config/Reader.pm \
	lib/Gitosis/Config/Writer.pm

PM_TO_BLIB = lib/Gitosis/Config/Writer.pm \
	blib/lib/Gitosis/Config/Writer.pm \
	lib/Gitosis/Config/Group.pm \
	blib/lib/Gitosis/Config/Group.pm \
	lib/Gitosis/Config/Reader.pm \
	blib/lib/Gitosis/Config/Reader.pm \
	lib/Gitosis/Config.pm \
...
DISTNAME = Gitosis-Config
DISTVNAME = Gitosis-Config-0.0.2
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Gitosis/Config.pm $(INST_MAN3DIR)/Gitosis::Config.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/00.load.t t/01.parse.t t/02.output.t
TESTDB_SW = -d
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Parse and Write gitosis config files</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Chris Prather  C&lt;&lt; &lt;perigrin@cpan.org&gt; &gt;&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Config-INI" VERSION="0,013,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-MakeMaker" VERSION="6,42,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Moose" VERSION="0,56,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MooseX-AttributeHelpers" VERSION="0,13,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Gitosis/Config/Writer.pm blib/lib/Gitosis/Config/Writer.pm \
	  lib/Gitosis/Config/Group.pm blib/lib/Gitosis/Config/Group.pm \
	  lib/Gitosis/Config/Reader.pm blib/lib/Gitosis/Config/Reader.pm \
	  lib/Gitosis/Config.pm blib/lib/Gitosis/Config.pm 
